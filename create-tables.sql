...
CREATE TABLE LOAN (
    ...
    LoanDate DATE,
    ReturnDate DATE,
    CONSTRAINT PK_Loan PRIMARY KEY (USERS, Copy, LoanDate),
    CONSTRAINT FK_Loan_User FOREIGN KEY (USERS) REFERENCES USERS(UserID) ON DELETE CASCADE,
    CONSTRAINT FK_Loan_Copy FOREIGN KEY (Copy) REFERENCES BOOKS(Copy_Signature) ON DELETE CASCADE
);
...
CREATE TABLE RESERVATION (
    ...
    CONSTRAINT FK_Reservation_Copy FOREIGN KEY (Copy) REFERENCES BOOKS(Copy_Signature) ON DELETE CASCADE
);
...
CREATE TABLE COMMENTS (
    ...
    CommentDate DATE,
    Vote INT DEFAULT 0,
    CONSTRAINT PK_Comment PRIMARY KEY (USERS, Copy_signature, CommentDate),
    CONSTRAINT FK_Comment_User FOREIGN KEY (USERS) REFERENCES USERS(UserID) ON DELETE CASCADE,
    CONSTRAINT FK_Comment_Book FOREIGN KEY (Copy_signature) REFERENCES BOOKS(Copy_signature) ON DELETE CASCADE
);
